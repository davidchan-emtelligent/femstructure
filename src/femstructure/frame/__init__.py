...
from ..utils.model_utils import (force_columns, parse_member_frame, parse_member_truss, parse_node, parse_restraint,
        ...
        if where == 'displacements':
            ...
            if isinstance(values, dict):
                for ntag, d in values.items():
                    rows += [[ntag] + [d[key] for key in force_columns]]
            else:
                for row in values:
